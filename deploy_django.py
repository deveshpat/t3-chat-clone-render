...
def test_django_app():
    ...
    success, _ = run_command("python manage.py check", "Running Django system check")
    if not success:
        return False
    
    # Test database
    success, _ = run_command("python manage.py showmigrations", "Checking database migrations")
    if not success:
        return False
